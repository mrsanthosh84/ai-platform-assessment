...
class MockOpenAI:
    """Mock OpenAI client for demo purposes when real client fails"""
    ...
    class Chat:
        class Completions:
            def create(self, model=None, messages=None, stream=False, **kwargs):
                ...
                elif "quit" in last_message or "bye" in last_message:
                    response_text = "Goodbye! Thanks for chatting with the demo AI."
                else:
                    response_text = f"I'm a demo AI assistant. You said: '{messages[-1]['content']}'. This is just a mock response for testing purposes."
                ...
                if stream:
                    ...
                    yield MockChunk(None)  # End of stream
                else:
...
class ChatBot:
    def __init__(self):
        try:
            # Try to import and use real OpenAI
            ...
            self.is_mock = True
            print("⚠️  Using mock AI (OpenAI library not compatible with Python 3.14)")
            print("   Install dependencies and use Docker for full functionality")
